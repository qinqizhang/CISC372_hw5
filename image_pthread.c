...
void *convolute(void *rank){
        ...
        long long my_first_i =my_rank*(srcImage.height+N-1)/N;
        long long my_last_i=my_first_i+(srcImage.height+N-1)/N;
	int row,pix,bit,span;
    	
    	for (row=my_first_i;row<my_last_i&&row<srcImage.height;row++){
		for (pix=0;pix<srcImage.width;pix++){
            		for (bit=0;bit<srcImage.bpp;bit++){
